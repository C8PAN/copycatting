...
namespace utils {
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline bool Inverse(Matrix4 &out) const {
			T tmp[12], src[16], det;
			T *dst = out.m;
			for (int i = 0; i < 4; i++) {
				src[i] = this->m[i * 4];
	...
	public:
		...
		// Matrix determinant
		_CPU_AND_GPU_CODE_ inline T Determinant() const {
		...
		// The inverse matrix for float/double type
		_CPU_AND_GPU_CODE_ inline bool Inverse(Matrix3 &out) const {
			T determinant = det();
			if (determinant == 0) {
				out.SetZeros();
				return false;
