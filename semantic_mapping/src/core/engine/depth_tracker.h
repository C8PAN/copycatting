...
public:
  ...
  DepthTracker(Vector2i image_size, TrackerIterationType* tracking_regime, 
...
private:
  ...
  void SetEvaluationParameters(int level_id);
  void ComputeDelta(float* delta, float* nabla, float* hessian, 
                    bool short_iteration) const;
  void ApplyDelta(const Matrix4f& old_parameter, const float* delta, 
                  Matrix4f& new_parameter) const;
  void HasConverged(float* step) const;
...
#endif /* end of include guard: DEPTH_TRACKER_H */
